...
file_ = open(r"C:\Users\TC\Documents\project\dna.gif", "rb")
contents = file_.read()
...
def local_sequence_alignment(s1, s2, match=1, mismatch=-1, gap=-2, dna_seq=True):
    if dna_seq:
        if sequence_checker(s1) and sequence_checker(s2):
            ...
            return align1[::-1], align2[::-1], matrix_del, score, path_pos[0], path
        else:
            return False
    else:
        matrix = np.zeros((len(s1) + 2, len(s2) + 2), dtype=int)
...
with st.sidebar:
    st.title("DNA Sequencing Classifier")
    ...
    if selected == 'Classifier':
        ...
        with header:
            st.title('Welcome to our DNA Classifier!')
        ...
        with datset:
            ...
            st.text('We found this data set at Keggle.com,\n'
        ...
        with features:
            ...
            st.markdown(
                '* Test Model Accuracy:This feature was made to enable the user to run tests on the model to realise the expected range of model accuracy')
        ...
        with model_training:
            ...
            if (selected == 'Classifier'):
                ...
                tnum = selection_col.slider('Choose the number of test to run on the model', min_value=1, max_value=10,
                                            value=5,
                                            step=1)
